...
RUN set -ex \
    && apk add --no-cache --virtual .build-deps libffi-dev zlib-dev jpeg-dev rust openssl-dev cargo python3-dev postgresql-dev build-base \
    && python -m venv /env \
    && python -m pip install -U pip \
    && /env/bin/pip install --upgrade pip \
    && /env/bin/pip install --no-cache-dir /srv/umap \
